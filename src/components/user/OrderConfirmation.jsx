...
function EventDetail() {
  ...
  const handleConfirmBooking = async () => {
    try {
      ...
      toast.success("Booking confirmed successfully,For confirm the order do the payment");
        ...
        {loading ? (
          ...
          <div className='bg-white rounded-lg shadow-lg p-6 grid grid-cols-2 gap-6'>
            <div className='relative group col-span-2 md:col-span-1'>
              <img className='w-full h-75 object-cover rounded-md' src={event.image} alt='event_image' />
              <div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity'>
                <h1 className='text-4xl font-Playball text-green-600'>{event.name}</h1>
                <p className='text-xl font-normal text-blue-600 mt-2'>{"₹ " + event.price_per_person + "/ per day"}</p>
              </div>
            </div>
            <div className='mt-10 md:mt-0 text-center col-span-2 md:col-span-1'>
               <h1 className='text-3xl font-semibold text-green-600'>{event.name}</h1> 
              <p className='text-gray-300 mt-2 mb-6 font-serif'>{event.description}</p>
              <div className='font-serif'>
                  <p className='text-gray-600'>category: {event.category.name}</p>
                 
                  <p className='text-gray-600'>Year Manufactured: {event.year_manufactured}</p>
                 
                  <p className='text-gray-600'>Seating Capacity: {event.seating_capacity}</p>
                  <p className='text-gray-600'>Servicer name: {event.servicer_name}</p>
                  <p className='text-gray-600'>veg/nonveg: {event.is_veg ? 'Yes' : 'No'}</p>
                </div>
                <div > 
              <p className='text-2xl font-serif text-primaryBlue'>Menus:</p>
                  <ul className='flex text-center'>
                    {menus.map((menu) => (
                      <li key={menu.id} className='text-gray-600 mr-4 font-serif border-customColorA border-s bg-slate-100 rounded-xl'>{menu.name}</li>
                    ))}
                  </ul>
              </div> 
</div>
...
<div className='flex flex-col bg-customColorD items-center justify-center mt-4 mx-auto max-w-[600px] font-serif'>
         <div className='card'>
        <h5 className="mt-1 text-slate-500 text-center"> Number of EventMembers:</h5>
     <input
          type="number"
        ...
        />
               ...
               {!showDate ? (
                <button
                  ...
                  onClick={() => {
                    if (!numberOfMembers || !requirements) {
                      toast.error("Please select both number of members and requirements");
                    } else {
                      toggleDate();
                    }
                  }}
                ...
                <div className='mb-4'>
                  ...
                  />
                     <h5 className="mt-1 font-serif text-red-200 text-xl">{dateAvailabilityStatus}</h5>
                  </div>
                  )
                }
                      ...
                      <div className="grid grid-cols-3 gap-4 mt-4">
                        {selectedSlots.map((slot) => {
                          ...
                          return (
                            <button
                              key={slot.id}
                              className={`bg-customColorD text-black py-2 px-4 rounded-md shadow-2xl ${slot.is_booked ? 'opacity-50 cursor-not-allowed' : ''}`}
                              id={slot.id}
