...
function Event1() {
      ...
      try {
        ...
        if (initialSearchQuery.trim() !== '') {
          const filteredEvents = events.data.filter(
            (event) =>
              event.name.toLowerCase().includes(initialSearchQuery.toLowerCase()) ||
              event.servicer.first_name.toLowerCase().includes(initialSearchQuery.toLowerCase())
          );
    ...
    if (eventsSearch.trim() === '') {
      ...
      const filteredEvents = eventsData.filter(
        (event) =>
          ...
          event.servicer_name.toLowerCase().includes(eventsSearch.toLowerCase())
      );
      setFilteredEventsData(filteredEvents);
    }
  };
  ...
  const handleCityChange = (event) => {
    ...
    if (selectedValue === '') {
      ...
      const filteredEvents = eventsData.filter((event) => event.servicer.city === selectedValue);
      setFilteredEventsData(filteredEvents);
    }
  };
  ...
  const handlePriceRangeChange = (event) => {
    ...
    if (selectedValue === '0-100') {
      filteredEvents = filteredEvents.filter((event) => event.starting_price < 100);
    } else if (selectedValue === '100-500') {
      filteredEvents = filteredEvents.filter((event) => event.starting_price >= 100 && event.starting_price <= 500);
    } else if (selectedValue === '500-1500') {
      filteredEvents = filteredEvents.filter((event) => event.starting_price >= 500 && event.starting_price <= 1500);
    } else if (selectedValue === '1500-2000') {
      filteredEvents = filteredEvents.filter((gig) => event.starting_price >= 15000 && event.starting_price <= 20000);
    } else if (selectedValue === '2000-above') {
      filteredEvents = filteredEvents.filter((event) => event.starting_price >= 2000);
    }
    setFilteredEventsData(filteredEvents);
  };
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex flex-wrap mt-5">
            ...
            <div className="group inline-block w-full sm:w-1/2 lg:w-1/4 mb-2 sm:mb-0">
                ...
                {uniqueCity && uniqueCity.map((city, index) => (
              ...
              >
                <Link to={`/single-view/${event.id}`}>
                  ...
                  <div className="text-center">
                    <p className="text-lg text-green-500 font-serif">{event.name}</p>
                  ...
                  <div className="mt-2 text-base text-gray-600 font-serif w-80 sm:w-64 md:w-80">
                    ...
                    <div className="flex flex-wrap justify-between">
                      <p className="w-1/2 text-black"> ₹{event.price_per_person}/person</p>
